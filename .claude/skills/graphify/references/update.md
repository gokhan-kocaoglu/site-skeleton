...
If `code_only` is False (any changed file is a doc/paper/image/video): **if any changed file is in `new_files['video']`, drop it from the run and tell the user video transcription is not included in this adaptation (install the official skill via `graphify install --project` for it)** — raw `.mp4/.mp3` paths must never be fed to semantic subagents as unreadable media. Then run the full Steps 3A–3C pipeline as normal on the remaining files.
